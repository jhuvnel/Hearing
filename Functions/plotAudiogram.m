...
sub_tab = AudioTab(contains(AudioTab.Subject,subs(indx)),:);
sub = subs{indx};
...
resp = plot_tab{:,5:2:21};
mask = plot_tab{:,6:2:22};
...
for i = 1:size(plot_tab,1)
    ...
    if contains(plot_tab.Type{i},'AC')
        if contains(plot_tab.Side{i},'Left')
            if any(out_bound)
        ...
        elseif contains(plot_tab.Side{i},'Right')
            if any(out_bound)
                %Make the response equal to the maximum threshold measured
                %by the audiometer 
                new_resp = [90,100,105,110,120,105,105,100,95]; %the air thresholds
    ...
    elseif contains(plot_tab.Type{i},'BC')
