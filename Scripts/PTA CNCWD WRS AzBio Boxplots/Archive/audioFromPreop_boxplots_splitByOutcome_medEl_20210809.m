...
hold on;
plotBPfromPreOp(mo6ArrayfromPreOp,groups)
...
function [] = plotBPfromPreOp(array,groups) % plot audiogram boxplot summaries from preop
boxWidth = 0.1;
freq = [125,250,500,1000,2000,4000,6000,8000];
...
for i = 1:length(freq)
    boxpos(i*2-1) = xpos(i) - xshift;
    boxpos(i*2) = xpos(i) + xshift;
end

bp = boxplot(array(groups==1,:),'Colors',colorArray,'Widths',boxWidth,'Positions',boxpos,'Symbol','','Whisker',3);
set(bp,'LineWidth',linew,'LineStyle','-');

plot(xpos-xshift,medianArrayHear(1:2:end),'k-','LineWidth',linew);
plot(xpos+xshift,medianArrayHear(2:2:end),':','LineWidth',linew,'Color',[0.2 0.2 0.2]);

bp = boxplot(array(groups==0,:),'Colors',colorArray,'Widths',boxWidth,'Positions',boxpos,'Symbol','','Whisker',3);
set(bp,'LineWidth',linew,'LineStyle','-');

plot(xpos-xshift,medianArrayNoHear(1:2:end),'k-','LineWidth',linew);
plot(xpos+xshift,medianArrayNoHear(2:2:end),':','LineWidth',linew,'Color',[0.2 0.2 0.2]);

for i = 1:length(array(:,1))
    for j = 1:2:length(array(1,:))
        text(xpos(j-floor(j/2))-markshift,array(i,j),shapes{i},'Color','k','HorizontalAlignment','center');
    end
    for j = 2:2:length(array(1,:))
        text(xpos(j-(j/2))+markshift,array(i,j),shapes{i},'Color',[0.2 0.2 0.2],'HorizontalAlignment','center');
    end
end
...
function [] = plotBP(array,groups) % plot audiogram boxplot summaries
boxWidth = 0.1;
freq = [125,250,500,1000,2000,4000,6000,8000];
shapes = {'A','B','C','D','E','F','G','H','I'};
xpos = log(freq);
