...
yr1ArrayfromPreOp = zeros(length(patients)-2,length(conduction)*length(freq));
yr1Array = zeros(length(patients)-2,length(conduction)*length(freq));
fontSize = 14;
...
hold on;
plotBPfromPreOp(mo6ArrayfromPreOp)
...
subplot(2,3,4);
hold on;
plotBPfromPreOp(mo1ArrayfromPreOp)
ylabel('Change from Pre-Op Threshold (dB HL)','FontSize',fontSize);

subplot(2,3,5);
hold on;
plotBPfromPreOp(mo2ArrayfromPreOp)

subplot(2,3,6);
hold on;
plotBPfromPreOp(yr1ArrayfromPreOp)
...
bp = boxplot(array,'Colors',colorArray,'Widths',boxWidth,'Positions',boxpos,'Symbol','');
set(bp,'LineWidth',linew,'LineStyle','-');

plot(xpos-xshift,medianArray(1:2:end),'k-','LineWidth',linew);
plot(xpos+xshift,medianArray(2:2:end),':','LineWidth',linew,'Color',[0.2 0.2 0.2]);
...
function [] = plotBP(array) % plot audiogram boxplot summaries
boxWidth = 0.1;
freq = [125,250,500,1000,2000,4000,6000,8000];
shapes = {'A','B','C','D','E','F','G','H'};
xpos = log(freq);
